...
class Ride
{
    ...
    {
        $this->db->query('SELECT posts.id,posts.source,posts.destination,posts.departure,posts.vehicle,posts.seats,posts.contact, posts.vehicle_id,posts.price,users.full_name FROM posts INNER JOIN users ON posts.user_id = users.id WHERE available=1 AND users.id <> :userid');
    ...
    public function addRide($data)
    {
        ...
        $this->db->query('INSERT INTO posts (user_id,source,destination,departure,vehicle,seats,contact,vehicle_id,price) VALUES (:user_id, :source, :destination, :departure, :vehicle, :seats, :contact, :vehicle_id, :price)');
